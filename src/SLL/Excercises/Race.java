...
public class Race {
    ...
    public static void competition(SLL<Athlete> prevWinners, SLL<Athlete> currYearRunners) {
        SLLNode<Athlete> firstLastYear = prevWinners.getFirst();
        double avr = 0.0;

        while (firstLastYear != null) {
            avr += firstLastYear.element.getTime();
            firstLastYear = firstLastYear.succ;
        }

        avr /= (double)prevWinners.length();

        SLLNode<Athlete> firstCurrYear = currYearRunners.getFirst();

        while (firstCurrYear != null) {
            if (firstCurrYear.element.getTime() > avr) {
                currYearRunners.delete(firstCurrYear);
            }
            firstCurrYear = firstCurrYear.succ;
        }
