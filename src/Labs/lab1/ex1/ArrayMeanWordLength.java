...
public class ArrayMeanWordLength {
    ...
    public static String wordClosestToAverageLength(Array<String> arr) {
        ...
        for (int i = 0; i < arr.getSize(); i++) {
            average += arr.get(i).length();
        }
        average = average / (double) arr.getSize();

        String closestString = arr.get(0);
        double closestLength = Math.abs(arr.get(0).length() - average);
        for (int i = 1; i < arr.getSize(); i++) {
            double minTemp = Math.abs(arr.get(i).length() - average);
            if (minTemp < closestLength && closestString.length() < arr.get(i).length()) {
                closestString = arr.get(i);
                closestLength = minTemp;
            }
        }
