...
public class MysticalCardGame {
    ...
    public static void startDuel(SLL<Card> firstSorcererCards, SLL<Card> secondSorcererCards) {
        ...
        while (temp != null) {
            if (bestCard.element.significance() < temp.element.significance())
                bestCard = temp;
            temp = temp.succ;
        }

        firstSorcererCards.delete(bestCard);
        SLLNode<Card> node = secondSorcererCards.getFirst();

        for (int i = 0; i < secondSorcererCards.size() / 2; i++) {
            node = node.succ;
        }

        secondSorcererCards.insertBefore(bestCard.element, node);
    }
